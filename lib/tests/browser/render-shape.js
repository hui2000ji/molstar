...
async function getMeshShape(ctx, data, props, shape) {
    ...
    return Shape.create('test', { ...data }, mesh, (groupId) => colors[groupId], // color: per group, same for instances
...
export async function init() {
    ...
    await meshRepr.createOrUpdate({ alpha: 0.5 }, meshData).run((p) => console.log(Progress.format(p)));
    console.log('mesh shape', meshRepr);
    canvas3d.add(meshRepr);
    // Create shape from spheresData and add to canvas3d
    await spheresRepr.createOrUpdate({}, spheresData).run((p) => console.log(Progress.format(p)));
