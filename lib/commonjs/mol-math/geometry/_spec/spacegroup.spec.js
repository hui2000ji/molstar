...
var construction_1 = require("../spacegroup/construction");
var linear_algebra_1 = require("../../linear-algebra");
function getSpacegroup(name) {
    var size = linear_algebra_1.Vec3.create(1, 1, 1);
    var anglesInRadians = linear_algebra_1.Vec3.create(Math.PI / 2, Math.PI / 2, Math.PI / 2);
    var cell = construction_1.SpacegroupCell.create(name, size, anglesInRadians);
...
function checkOperatorsXyz(name, expected) {
    ...
    for (var i = 0, il = spacegroup.operators.length; i < il; ++i) {
        ...
        var actual = construction_1.Spacegroup.getOperatorXyz(op);
