...
async function readCIF(path) {
    ...
    const models = await (0, mmcif_2.trajectoryFromMmCIF)(data).run();
...
(function (PropertyAccess) {
    ...
    async function testAssembly(id, s) {
        console.time('assembly');
        const a = await structure_1.StructureSymmetry.buildAssembly(s, '1').run();
    ...
    async function testSymmetry(id, s) {
        console.time('symmetry');
        const a = await structure_1.StructureSymmetry.buildSymmetryRange(s, linear_algebra_1.Vec3.create(-1, -1, -1), linear_algebra_1.Vec3.create(1, 1, 1)).run();
        // const auth_comp_id = SP.residue.auth_comp_id;
        // const q1 = Query(Q.generators.atoms({ residueTest: l => auth_comp_id(l) === 'ALA' }));
        // const alas = await query(q1, a);
    ...
    async function testIncludeSurroundings(id, s) {
        // const a = s;
        console.time('symmetry');
        const a = await structure_1.StructureSymmetry.buildSymmetryRange(s, linear_algebra_1.Vec3.create(-2, -2, -2), linear_algebra_1.Vec3.create(2, 2, 2)).run();
        // console.log(printUnits(a));
        const auth_comp_id = structure_1.StructureProperties.atom.auth_comp_id, op = structure_1.StructureProperties.unit.operator_name;
        // const q1 = Q.generators.atoms({ residueTest: l => auth_comp_id(l) === 'REA' });
        const q1 = structure_1.Queries.modifiers.includeSurroundings(structure_1.Queries.generators.atoms({
        ...
        const surr = structure_1.StructureSelection.unionStructure(structure_1.StructureQuery.run(q1, a));
        console.timeEnd('symmetry');
    ...
    async function run() {
        ...
        const auth_comp_id = structure_1.StructureProperties.atom.auth_comp_id;
        // const auth_asym_id = SP.chain.auth_asym_id;
        // const set =  new Set(['A', 'B', 'C', 'D']);
        // const q = Q.generators.atomGroups({ atomTest: l => auth_seq_id(l) < 3 });
        const q = structure_1.Queries.generators.atoms({ atomTest: structure_1.Queries.pred.eq(l => structure_1.StructureProperties.atom.auth_comp_id(l.element), 'ALA') });
        const P = structure_1.StructureProperties;
        // const q0 = Q.generators.atoms({ atomTest: l => auth_comp_id(l) === 'ALA' });
        const q1 = (structure_1.Queries.generators.atoms({ atomTest: l => auth_comp_id(l.element) === 'ALA' }));
        const q2 = (structure_1.Queries.generators.atoms({ atomTest: l => auth_comp_id(l.element) === 'ALA', groupBy: l => structure_1.StructureProperties.residue.key(l.element) }));
        const q3 = (structure_1.Queries.generators.atoms({
            chainTest: structure_1.Queries.pred.inSet(l => P.chain.auth_asym_id(l.element), ['A', 'B', 'C', 'D']),
            atomTest: structure_1.Queries.pred.eq(l => P.atom.auth_comp_id(l.element), 'ALA')
        }));
...
})(PropertyAccess || (exports.PropertyAccess = PropertyAccess = {}));
