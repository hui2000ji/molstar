...
exports.IntVsStringIndices = exports.ObjectVsMap = exports.testSegments = exports.Tuples = exports.Union = void 0;
var tslib_1 = require("tslib");
var B = tslib_1.__importStar(require("benchmark"));
...
(function (Union) {
    ...
    function createData(array) {
        ...
        return { ordSet: int_1.OrderedSet.ofSortedArray(array), obj: obj, set: set };
    }
    function unionOS(a, b) {
        return int_1.OrderedSet.union(a, b);
    }
    function intOS(a, b) {
        return int_1.OrderedSet.intersect(a, b);
    }
    function unionO(a, b) {
    ...
    function run() {
        ...
        console.log(int_1.OrderedSet.size(unionOS(osA, osB)), Object.keys(unionO(oA, oB)).length, unionS(sA, sB).size);
        console.log(int_1.OrderedSet.size(intOS(osA, osB)), Object.keys(intO(oA, oB)).length, intS(sA, sB).size);
        suite
...
(function (Tuples) {
    function createData(n) {
        ...
        for (var i = 0; i < n; i++) {
            ret[i] = int_1.Tuple.create(i, i * i + 1);
        }
        return ret;
    }
    function sum1(data) {
        ...
        for (var i = 0, _i = data.length; i < _i; i++) {
            s += int_1.Tuple.fst(data[i]) + int_1.Tuple.snd(data[i]);
        }
        return s;
    }
    function sum2(data) {
        ...
        for (var i = 0, _i = data.length; i < _i; i++) {
            var t = data[i];
            s += int_1.Tuple.fst(t) + int_1.Tuple.snd(t);
        }
        return s;
    }
    function run() {
    ...
    Tuples.run = run;
})(Tuples || (exports.Tuples = Tuples = {}));
function testSegments() {
    var data = int_1.OrderedSet.ofSortedArray([4, 9, 10, 11, 14, 15, 16]);
    var segs = int_1.Segmentation.create([0, 4, 10, 12, 13, 15, 25]);
    var it = int_1.Segmentation.transientSegments(segs, data);
    while (it.hasNext) {
        var s = it.move();
        for (var j = s.start; j < s.end; j++) {
            console.log("".concat(s.index, ": ").concat(int_1.OrderedSet.getAt(data, j)));
        }
    }
}
exports.testSegments = testSegments;
var ObjectVsMap;
(function (ObjectVsMap) {
    ...
    ObjectVsMap.run = run;
})(ObjectVsMap || (exports.ObjectVsMap = ObjectVsMap = {}));
var IntVsStringIndices;
...
})(IntVsStringIndices || (exports.IntVsStringIndices = IntVsStringIndices = {}));
